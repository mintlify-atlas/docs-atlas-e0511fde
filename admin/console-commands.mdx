---
title: "CollaboKeys Console Command Reference"
...
```bash
disable emulation
disable d e       # same as above using command alias + arg alias
disable reservation
disable r         # same as above (alias)
```
...
```bash
key <revoke|enable|disable> <keyname|all>
```
...
<ParamField path="target" type="string" required>
  ...
  - `all` — applies the action to every key in the keycodes table
</ParamField>
...
| `key` | `k` | `<revoke\|enable\|disable> <keyname\|all>` | Manage key assignment or enabled status |
